...
def get_cub_models(args, iteration):
    ...
    bb.load_state_dict(
        torch.load(
            "Path/PhD/miccai/checkpoints/cub/BB/lr_0.03_epochs_95/ViT-B_16/VIT_CUBS_8000_checkpoint.bin"
        )["model"]
    )
    bb.eval()
    ...
    t_model.load_state_dict(
        torch.load(
            "Path/PhD/miccai/checkpoints/cub/t/lr_0.03_epochs_95/lr_0.03_epochs_95_ViT-B_16_layer4_VIT_sgd_BCE/g_best_model_epoch_54.pth.tar"
        )
    )
    t_model.eval()
