...
from Explainer.experiments_explainer_mimic_cxr import train_glt, test_baseline_post_hoc, \
    train_baseline_LR_post_hoc
...
parser.add_argument('--image-path-ocean-shared', metavar='DIR',
                    default='Path/Projects/ImgTxtWeakSupLocalization/CXR_Datasets/MIMICCXR/files/mimic-cxr-jpg/2.0.0/files',
                    help='image path in ocean')
parser.add_argument('--dataset-dir', metavar='DIR',
                    default='Path/PythonProject/MIMIC-CXR-SSL/',
                    help='dataset directory')
parser.add_argument('--img-chexpert-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/mimic-cxr-chexpert.csv',
...
parser.add_argument('--radgraph-adj-mtx-pickle-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/landmark_observation_adj_mtx_v2.pickle',
                    help='radgraph adjacent matrix landmark - observation.')
parser.add_argument('--radgraph-sids-npy-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/landmark_observation_sids_v2.npy',
                    help='radgraph study ids.')
parser.add_argument('--radgraph-adj-mtx-npy-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/landmark_observation_adj_mtx_v2.npy',
                    help='radgraph adjacent matrix landmark - observation.')
parser.add_argument('--nvidia-bounding-box-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/mimic-cxr-annotation.csv',
...
parser.add_argument('--imagenome-radgraph-landmark-mapping-file', metavar='PATH',
                    default='Path/PythonProject/MIMIC-CXR-SSL/landmark_mapping.json',
...
parser.add_argument('--logs', metavar='DIR',
                    default='Path/PhD/miccai/log',
                    help='path to tensorboard logs')
parser.add_argument('--checkpoints', metavar='DIR',
                    default='Path/PhD/miccai/checkpoints',
...
def main():
    ...
    if args.train_baseline == "y":
        print("Training baseline for MIMIC-CXR")
        best_aurocs = []
        for seed in range(args.seed):
            ...
            print("###" * 25)
            args.cur_seed = seed
            best_auroc = train_baseline_LR_post_hoc(args)
            # best_auroc = train_baseline_post_hoc(args)
            print(f"===============>>>>> best_auroc: {best_auroc} <<<<<===============")
            print("###" * 25)
            print(f">>>>>>>>>>>>>> End seed: {seed} <<<<<<<<<<<<<<<")
            print("###" * 25)
            best_aurocs.append(best_auroc)
        print(best_aurocs)
        print(f"Mean: {np.mean(np.array(best_aurocs))}, std_dev: {np.std(np.array(best_aurocs))}")
    elif args.train_baseline == "n":
        print("Testing baseline for MIMIC-CXR")
        for seed in range(args.seed):
            print("###" * 25)
            print(f">>>>>>>>>>>>>> Start seed: {seed} <<<<<<<<<<<<<<<")
            print("###" * 25)
