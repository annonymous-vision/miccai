...
def test_baseline_post_hoc(args):
    ...
    root = f"{args.arch}_lr_{args.lr}_{args.optim}_temperature-lens_{args.temperature_lens}" \
           f"lambda-lens_{args.lambda_lens}" \
           f"_hidden-layers_{hidden_layers}" \
    ...
    g_chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline_PostHoc", args.arch, disease_folder, root,
                                 f"seed_{args.cur_seed}")
    g_output_path = os.path.join(args.output, args.dataset, "Baseline_PostHoc", args.arch, disease_folder, root,
                                 f"seed_{args.cur_seed}")
    g_tb_logs_path = os.path.join(args.logs, args.dataset, "Baseline_PostHoc", args.arch, disease_folder, root,
                                  f"seed_{args.cur_seed}")
    ...
    cur_glt_chkpt = os.path.join(g_chk_pt_path, "model_seq_epoch_4.pth.tar")
...
def predict_baseline(
    ...
    out_put_class = out_put_class.cpu()
    out_put_class_bb = out_put_class_bb.cpu()
    ...
    print(f"out_put_class_pred size: {out_put_class.size()}")
    ...
    utils.save_tensor(
        path=os.path.join(output_path, f"{mode}_out_put_class_pred.pt"), tensor_to_save=out_put_class
    )
    utils.save_tensor(
        path=os.path.join(output_path, f"{mode}_out_put_class_bb_pred.pt"), tensor_to_save=out_put_class_bb
    )
    ...
    results_arr = []
    ii = 0
    for _idx in range(out_put_class.size(0)):
        ii += 1
        print(f"===>> {ii} <<===")
    ...
    pickle.dump(results_arr, open(os.path.join(output_path, f"{mode}_FOL_results_baseline_pcbm.pkl"), "wb"))
    test_results_df = pd.DataFrame.from_dict(results_arr, orient='columns')
    test_results_df.to_csv(os.path.join(output_path, f"{mode}_FOL_results_baseline_pcbm.csv"))


def train_baseline_LR_post_hoc(args):
    print("************* Training baseline as per LR ************* ")
    random.seed(args.cur_seed)
    np.random.seed(args.cur_seed)
    torch.manual_seed(args.cur_seed)
    ...
    g_chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline_PostHoc_LR", args.arch, disease_folder, root,
                                 f"seed_{args.cur_seed}")
    g_output_path = os.path.join(args.output, args.dataset, "Baseline_PostHoc_LR", args.arch, disease_folder, root,
                                 f"seed_{args.cur_seed}")
    g_tb_logs_path = os.path.join(args.logs, args.dataset, "Baseline_PostHoc_LR", args.arch, disease_folder, root,
                                  f"seed_{args.cur_seed}")
    ...
    print(g_tb_logs_path)
    print("############# Paths ############# ")
    model = PCBM(ip_size=len(args.concept_names), op_size=len(args.labels)).to(device)
    ...
    logger = Logger_MIMIC_CXR(
        1, best_auroc, args.start_epoch, g_chk_pt_path, g_tb_logs_path, g_output_path, train_loader, val_loader,
        n_class, model_type="g", device=device
    )

    return fit_baseline_LR(
        args,
        model,
        optimizer,
        train_loader,
...
def fit_baseline_LR(
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch_id, data in enumerate(train_loader):
                ...
                train_loss = criterion(y_hat, train_y) + 0.00001 * (
                        0.99 * model.model.weight.abs().sum() + (1 - 0.99) * model.model.weight.pow(
                    2).sum())
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader)) as t:
                for batch_id, data in enumerate(val_loader):
                    ...
                    val_loss = criterion(y_hat, valid_y) + 0.00001 * (
                            0.99 * model.model.weight.abs().sum() + (1 - 0.99) * model.model.weight.pow(
                        2).sum())
    ...
    logger.end_run()
    return round(logger.best_auroc, 4)
