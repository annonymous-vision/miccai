...
def train_cbm_joint(args):
    ...
    for seed in range(args.seed):
        ...
        if args.arch == "ResNet101" or args.arch == "ResNet50":
        ...
        solver_c = torch.optim.SGD(
            baseline.parameters(),
            lr=args.lr,
            momentum=args.momentum,
            weight_decay=args.weight_decay
        )
        solver_g = torch.optim.SGD(g.explainer.parameters(), lr=args.lr_explainer, momentum=0.9, weight_decay=5e-4)
...
def fit_explainer_joint(
    ...
    for epoch in range(epochs_g):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch_id, data_tuple in enumerate(train_loader):
                if dataset == "cub":
                    ...
                    image, attribute, _, label = data_tuple
                ...
                y_hat, _, _ = g(torch.sigmoid(logits_concepts))

                c_loss = criterion_c(logits_concepts, attribute)
                y_loss = criterion_g(y_hat, label)
                train_loss = y_loss + _lambda_joint * c_loss
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader)) as t:
                for batch_id, data_tuple in enumerate(val_loader):
                    ...
                    c_loss = criterion_c(logits_concepts, attribute)
                    y_loss = criterion_g(y_hat, label)
                    val_loss = y_loss + _lambda_joint * c_loss
        ...
        print(f"Epoch: [{epoch + 1}/{epochs_g}] "
...
def test_explainer(args):
    ...
    for seed in range(args.seed):
        ...
        root = f"lr_{args.lr}_epochs_{args.epochs}"
        if args.dataset == "cub":
            chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline", args.arch, "explainer")
            output_path = os.path.join(args.output, args.dataset, "Baseline", args.arch, "explainer")
            backbone_chk_pt = os.path.join(
                args.checkpoints, args.dataset, "Baseline", args.arch, "Backbone"
            )
        elif args.dataset == "awa2":
            ...
            backbone_chk_pt = os.path.join(
                args.checkpoints, args.dataset, "Baseline", root, args.arch
            )
        ...
        device = utils.get_device()
        print(f"Device: {device}")
        args.g_output_path = output_path
        args.g_chk_pt_path = chk_pt_path
        ...
        if args.dataset == "cub":
            ...
            train_dataset = torch.utils.data.Subset(dataset, train_indices)
            train_ds_awa2 = Awa2(train_dataset, val_transform)
            train_loader = DataLoader(train_ds_awa2, batch_size=args.bs, shuffle=False, num_workers=4, pin_memory=True)
        ...
        if args.arch == "ResNet101" or args.arch == "ResNet50":
            baseline = BaseLine_CUB(args, test=True).to(device)
            baseline.load_state_dict(torch.load(os.path.join(backbone_chk_pt, args.bb_chkpt_file)))
        else:
            baseline = VisionTransformer_baseline(
                CONFIGS[args.arch], args.img_size, zero_head=True, num_classes=len(args.labels),
                op_size=len(args.concept_names), smoothing_value=args.smoothing_value, test=True
            ).to(device)
            baseline.load_state_dict(torch.load(os.path.join(backbone_chk_pt, args.bb_chkpt_file))["model"])

        baseline.eval()
        args.input_size_pi = 2048
        print(f"Checkpoint path: {chk_pt_path}")
        print(f"Output path: {output_path}")
        cur_glt_chkpt = os.path.join(chk_pt_path, args.g_checkpoint)
        g = Gated_Logic_Net(
            args.input_size_pi,
            args.concept_names,
            args.labels,
...
def predict(
    ...
    with torch.no_grad():
        with tqdm(total=len(val_loader)) as t:
            for batch_id, data_tuple in enumerate(val_loader):
                if dataset == "cub":
                    image, label, attributes = data_tuple
                elif dataset == "awa2":
                    image, attributes, _, label = data_tuple
                image, label, attributes = image.to(device), label.to(torch.long).to(device), attributes.to(
                    torch.float32).to(device)
                logits_concepts, phi = baseline(image)
                y_hat, selection_out, auxiliary_out, concept_mask, \
                alpha, alpha_norm, \
                conceptizator = g(torch.sigmoid(logits_concepts), test=True)

                t.set_postfix(epoch='{0}'.format(batch_id))
                t.update()

                # tensor_images = torch.cat((tensor_images, image.cpu()), dim=0)
                tensor_features = torch.cat((tensor_features, phi.cpu()), dim=0)
                tensor_concepts = torch.cat((tensor_concepts, torch.sigmoid(logits_concepts)), dim=0)
                tensor_preds = torch.cat((tensor_preds, y_hat), dim=0)
                tensor_attributes = torch.cat((tensor_attributes, attributes), dim=0)
    ...
    acc = torch.sum(tensor_preds.argmax(dim=1) == tensor_y) / tensor_preds.size(0)
...
def train_explainer(args):
    ...
    for seed in range(args.seed):
        ...
        print("==============================================")
        root = f"lr_{args.lr}_epochs_{args.epochs}"
        chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline", f"seed_{seed}", args.arch, "explainer")
        output_path = os.path.join(args.output, args.dataset, "Baseline", f"seed_{seed}", args.arch, "explainer")
        tb_logs_path = os.path.join(
            args.logs, args.dataset, "Baseline", f"seed_{seed}", f"{root}_{args.arch}", "explainer"
        )
        os.makedirs(chk_pt_path, exist_ok=True)
        os.makedirs(output_path, exist_ok=True)
        os.makedirs(tb_logs_path, exist_ok=True)
        ...
        # uncomment this
        # backbone_chk_pt = os.path.join(
        #     args.checkpoints, args.dataset, "Baseline", f"seed_{seed}", args.arch, "Backbone"
        # )
        backbone_chk_pt = os.path.join(
            args.checkpoints, args.dataset, "Baseline", args.arch, "Backbone"
        )
        print(backbone_chk_pt)
        if args.arch == "ResNet101" or args.arch == "ResNet50":
            baseline = BaseLine_CUB(args).to(device)
            baseline.load_state_dict(torch.load(os.path.join(backbone_chk_pt, args.bb_chkpt_file)))
        else:
...
def fit_explainer(
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch_id, data_tuple in enumerate(train_loader):
                if dataset == "cub":
                    image, label, _ = data_tuple
                elif dataset == "awa2":
                    image, _, _, label = data_tuple
                image, label = image.to(device), label.to(torch.long).to(device)
                solver.zero_grad()
...
def train_backbone(args):
    ...
    for seed in range(args.seed):
        ...
        print("==============================================")
        root = f"lr_{args.lr}_epochs_{args.epochs}"
        chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline", f"seed_{seed}", args.arch, "Backbone")
        output_path = os.path.join(args.output, args.dataset, "Baseline", f"seed_{seed}", args.arch, "Backbone")
        tb_logs_path = os.path.join(
            args.logs, args.dataset, "Baseline", f"seed_{seed}", f"{root}_{args.arch}", "Backbone"
        )
        os.makedirs(chk_pt_path, exist_ok=True)
        os.makedirs(output_path, exist_ok=True)
        os.makedirs(tb_logs_path, exist_ok=True)
