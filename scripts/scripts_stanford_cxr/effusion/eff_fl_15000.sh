...
python Path/PhD/miccai/codebase/cal_flops_cxr_domain_transfer_main.py \
  --disease "effusion" \
  --tot_samples 15000 \
  --batch-size 64 \
  --epochs 5 --profile "y" >$slurm_output_moie

python Path/PhD/miccai/codebase/train_BB_stanford_cxr.py --arch='densenet121' --selected-obs="effusion" --epochs 1 --labels "0 (No effusion)" "1 (effusion)"  --domain_transfer "y" --tot_samples 15000 --checkpoint-mimic-cxr "g_best_model_epoch_3.pth.tar" --profile "y" >$slurm_output_BB
