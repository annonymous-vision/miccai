...
python Path/PhD/miccai/codebase/train_BB_stanford_cxr.py --arch='densenet121' --selected-obs="cardiomegaly" --epochs 5 --labels "0 (No cardiomegaly)" "1 (cardiomegaly)"  --domain_transfer "y" --tot_samples 15000 --checkpoint-mimic-cxr "g_best_model_epoch_2.pth.tar" > $slurm_output_bb
...
python Path/PhD/miccai/codebase/train_explainer_cxr_domain_transfer.py \
  --target_dataset "stanford_cxr" \
  --disease "cardiomegaly" \
  --tot_samples 15000 \
  --batch-size 64 \
...
python Path/PhD/miccai/codebase/performance_calculation_cxr_domain_transfer_main.py --iterations 3 --disease "cardiomegaly" --model "MoIE" --tot_samples 15000 --cov 0.4 0.3 0.3 --initialize_w_mimic "y" > $slurm_output_total
